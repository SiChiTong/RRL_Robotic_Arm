...
void JointsController::Init(gazebo::physics::JointControllerPtr jointController,ArmStructure *armStructure ,gazebo::physics::WorldPtr world, std::map<std::string,  double>& dictionary)
{
  ...
  this->initPids(dictionary["$p"], dictionary["$i"], dictionary["$d"], dictionary["$imax"], dictionary["$max"], dictionary["$zfactor"]);
...
void JointsController::initPids(double p, double i, double d, double imax, double cmdMax, double zfactor)
{
  ...
  for(auto it = joints.begin(); it != joints.end(); it++)
  {
    ...
    {
      gazebo::common::PID PID = gazebo::common::PID(zfactor * p,i,d,imax,-1 * imax ,cmdMax,-1 * zfactor * cmdMax);
      this->jointController->SetPositionPID((*it)->GetScopedName(),PID);
    }else{
      gazebo::common::PID PID = gazebo::common::PID(p,i,d,imax, -1* imax, cmdMax, -1*cmdMax);
      this->jointController->SetPositionPID((*it)->GetScopedName(),PID);
    }
  }
}
...
void JointsController::SetRelativeTargets(const std::string& name, const std::vector<double>& targets)
{
...
bool JointsController::AreTargetsReached(const std::string& name, bool strict)
{
  std::vector<gazebo::physics::JointPtr> joints = this->armStructure->GetJoints(name);
