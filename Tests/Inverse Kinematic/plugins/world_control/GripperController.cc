...
void GripperController::Init(gazebo::physics::JointControllerPtr jointController,ArmStructure *armStructure ,std::map<std::string, double> dictionary)
{
...
// COULD BE IMPROVED (speed depend on update rate and step size)
...
// Could be improved (risk of pushing to hard / same reason as approach)
bool GripperController::stabilization()
{
  ...
  for(unsigned int i = 0; i < this->joints.size(); i++)
  {
    ...
    {
      //if(std::fabs(this->joints[i]->GetAngle(0).Radian() - this->targets[i]) < this->converror)
      {
