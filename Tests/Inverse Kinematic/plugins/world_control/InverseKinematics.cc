...
std::vector<double> InverseKinematics::CCD(std::vector<gazebo::math::Pose> poses, std::vector<std::vector<double>> jointsLimits, std::vector<gazebo::math::Vector3> endEffectors, std::vector<gazebo::math::Vector3> targets)
{
  ...
  while( iter < 1 || ( ( this->getError("max",this->endEffectors) > this->errormax ) && ( iter < this->itermax )  && ( previous_error_mean > (this->getError("mean",this->endEffectors) + this->converror) || previous_error_max > (this->getError("max",this->endEffectors) + this->converror)) ) )
  {
  ...
  {
    this->statistics->WriteInFile("ik", this->endEffectors[0]);this->statistics->WriteInFile("ik", ";");
    this->statistics->WriteInFile("ik", iter);this->statistics->WriteInFile("ik", "\n");
  }
