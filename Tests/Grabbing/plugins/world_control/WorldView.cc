...
void WorldView::SetTimer(std::string name, float seconds)
{
...
bool WorldView::IsTimerOff(std::string name)
{
...
std::vector<gazebo::math::Vector3> WorldView::GetBoxGrabbingPoints(std::string objname, double offset)
{
  // TO REMOVE
...
std::vector<gazebo::math::Vector3> WorldView::GetBoundingBoxGrabbingPoints(std::string objname, double offset)
{
  gazebo::physics::ModelPtr obj = this->world->GetModel(objname);
  gazebo::math::Pose objpose = obj->GetWorldPose();
  gazebo::math::Vector3 objpos = objpose.pos; // absolute centroid position of obj
