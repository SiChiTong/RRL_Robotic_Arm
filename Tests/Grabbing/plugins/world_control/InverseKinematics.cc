...
void InverseKinematics::Init(std::vector<gazebo::math::Vector3> axis, std::vector<std::vector<unsigned int>> children, std::vector<unsigned int> endEffJoints, std::map<std::string, double> dictionary, Statistics *statistics)
{
...
bool InverseKinematics::updateKinematicParameters(std::vector<gazebo::math::Pose> poses, std::vector<std::vector<double>> jointsLimits, std::vector<gazebo::math::Vector3> endEffectors, std::vector<gazebo::math::Vector3> targets)
{
...
std::vector<double> InverseKinematics::CCD(std::vector<gazebo::math::Pose> poses, std::vector<std::vector<double>> jointsLimits, std::vector<gazebo::math::Vector3> endEffectors, std::vector<gazebo::math::Vector3> targets)
{
  ...
  while( iter < 10 || ( ( this->getError("max",this->endEffectors) > this->errormax ) && ( iter < this->itermax )  && ( previous_error_mean > (this->getError("mean",this->endEffectors) + this->converror) || previous_error_max > (this->getError("max",this->endEffectors) + this->converror)) ) )
  {
  ...
  {
    this->statistics->WriteInFile("grabbox", "ik;");
    this->statistics->WriteInFile("grabbox", iter);
    this->statistics->WriteInFile("grabbox", ";");
    this->statistics->WriteInFile("grabbox", this->getError("mean",this->endEffectors));
    this->statistics->WriteInFile("grabbox", ";");
    this->statistics->WriteInFile("grabbox", this->getError("max",this->endEffectors));
    this->statistics->WriteInFile("grabbox", "\n");
  }
...
double InverseKinematics::computeDescent(unsigned int i, gazebo::math::Vector3 endEffector, gazebo::math::Vector3 target)
{
...
std::vector<double> InverseKinematics::normalizeAngles(std::vector<double> angles)
{
...
void InverseKinematics::forwardPass(unsigned int i, unsigned int j, gazebo::math::Quaternion quater)
{
  // Update pos
...
std::vector<unsigned int> InverseKinematics::uniqueConcat(std::vector<unsigned int> A, std::vector<unsigned int> B)
{
  if(A.size() == 0)
  {
    return B;
...
double InverseKinematics::get3DAngle(gazebo::math::Vector3 v1, gazebo::math::Vector3 v2)
{
...
double InverseKinematics::getError(std::string errortype, std::vector<gazebo::math::Vector3> endEffectors)
{
  if(errortype == "mean")
  {
...
double InverseKinematics::getMaxError(std::vector<gazebo::math::Vector3> endEffectors)
{
  double error = 0;
  for(unsigned int i=0; i<this->targetsNumber; i++)
  {
...
double InverseKinematics::getMeanError(std::vector<gazebo::math::Vector3> endEffectors)
{
  double error = 0;
  for(unsigned int i=0; i<this->targetsNumber; i++)
  {
