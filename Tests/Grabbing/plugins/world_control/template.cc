...
std::string convertFormulas(std::string converted_template)
{
  std::size_t pos = 0;
  char formulaMark = '&';
  while( (pos = converted_template.find(formulaMark) ) != std::string::npos)
  {
    // Get the entire formula
    std::string stringformula = extractFormula(converted_template, pos);
    ...
    converted_template.replace(pos,stringformula.length(), str);
  }
...
std::string convertVariables(std::string string_template, std::map<std::string, double> dictionary)
{
  ...
  {
    while( (pos = string_template.find(it->first) ) != std::string::npos)
    {
      std::ostringstream strs;
      strs << it->second;
      const std::string str = strs.str();
      string_template.replace( pos, (it->first).length(), str);
    }
  }
